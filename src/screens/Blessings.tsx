...
const BLESSING_GROUP_ORDER: BlessingGroup[] = [
  BlessingGroup.LIGHT,
  BlessingGroup.DARK,
  BlessingGroup.WAR,
  BlessingGroup.CHAOS,
  BlessingGroup.HARMONY,
];
...
// list of everyone in a modal instead of expanding in place — a blessing
// used by 40 champions would otherwise wrap across many lines and blow out
// that one card's height relative to its siblings in the grid.
...
export default function Blessings() {
  ...
  const championsByBlessing = useMemo(() => {
    const map = new Map<string, IChampion[]>();
    for (const c of champions) {
      if (!c.blessing) continue;
      const list = map.get(c.blessing) ?? [];
      list.push(c);
      map.set(c.blessing, list);
    }
  ...
  const filteredBlessings = useMemo(() => {
    ...
    if (!lower) return BLESSINGS;
    return BLESSINGS.filter((b) => b.name.toLowerCase().includes(lower));
  }, [search]);

  const equippedCount = champions.filter((c) => !!c.blessing).length;

  if (loading) return <ArcaneLoader label="Loading blessings" />;
  ...
  return (
    <div className="overflow-auto h-[92vh] p-4 space-y-6">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-xl font-bold flex items-center gap-2">
            <GiHolySymbol className="text-amber-500" size={22} />
            Blessings
          </h1>
          <p className="text-xs text-gray-400 mt-0.5">
            All {BLESSINGS.length} blessings — {equippedCount} of your champions currently have one equipped.
          </p>
        </div>
        <input
          value={search}
      ...
      <div className="space-y-6">
        {BLESSING_GROUP_ORDER.map((group) => {
          const blessingsInGroup = filteredBlessings.filter((b) => b.group === group);
          ...
          return (
            <section key={group} className="space-y-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                {blessingsInGroup.map((blessing) => {
                  const users = championsByBlessing.get(blessing.id) ?? [];
                  return (
                    <div
                      key={blessing.id}
                    ...
                    >
                      <img
                        src={getBlessingImagePath(blessing.id)}
                        alt={blessing.name}
                        className={`w-14 h-14 rounded-lg object-cover border-2 shrink-0 ${RARITY_BORDER_COLOR[blessing.rarity]}`}
                      />
                      <div className="flex-1 min-w-0 space-y-1">
                        <div className="flex items-center justify-between gap-2">
                          <p className="font-semibold text-sm truncate">{blessing.name}</p>
                          <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded shrink-0 ${RARITY_BADGE[blessing.rarity] ?? ""}`}>
                            {blessing.rarity}
                          </span>
                        </div>
                        <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-2">{blessing.description}</p>

                        <EquippedByList itemName={blessing.name} users={users} onPreview={setPreviewChampion} />
                      </div>
                    </div>
                  );
                })}
